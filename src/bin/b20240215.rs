...
impl Solution {
    pub fn find_relative_ranks(score: Vec<i32>) -> Vec<String> {
        ...
        let mut map: Vec<(usize, &i32)> = score.iter().enumerate().map(|(k, v)| (k, v)).collect();
    ...
    pub fn is_happy(n: i32) -> bool {
        ...
        while !is_done(&digits) {
            let sum: i32 = digits.iter().map(|x| x * x).sum();
    ...
    pub fn sorted_squares(nums: Vec<i32>) -> Vec<i32> {
        let mut answer: Vec<i32> = nums.iter().map(|x| x * x).collect();
